...
function Step2Interview({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    ...
    if (isAIPlaying) return; // Don't count down while AI is speaking
  ...
  // Browser speech recognition (live preview only)
  useEffect(() => {
  ...
  const startMic = async () => {
    ...
    // Start MediaRecorder for accurate Groq transcription
  ...
  const submitAnswer = async () => {
    ...
    try {
      // Transcribe with Groq Whisper for accurate text
        ...
        <div className='flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative'>
          ...
          {isMicOn && !isAIPlaying && !feedback && !isIntroPhase && (
            <div className='flex items-center gap-2 mb-3 px-2'>
              ...
              <span className='text-xs text-gray-500 font-medium'>Recording with Groq Whisper • High accuracy STT</span>
            </div>
          )}
